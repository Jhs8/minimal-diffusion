...
def train_one_epoch(
    ...
    for step, (images, labels) in enumerate(dataloader):
        ...
        if args.local_rank == 0:
            ...
            logger.log(loss.item(), display=(step % 200 == 0))
...
def sample_N_images(
        ...
        while num_samples < N:
            ...
            if args.class_cond!=-2:
                if args.class_cond==-1:
                    ...
                    y = torch.ones(len(xT), dtype=torch.int64).to(args.device) * args.class_cond
            else:
                y = None
    ...
    return (samples, np.concatenate(labels) if args.class_cond!=-2 else None)
...
def main():
    ...
    for epoch in range(501,args.epochs+501):
        ...
        if not epoch % 50:
            ...
            if args.local_rank == 0:
                cv2.imwrite(
                    os.path.join(
                        args.save_dir,
                        f"{args.arch}128_{args.dataset}-{args.diffusion_steps}_steps-{args.sampling_steps}-sampling_steps-class_condn_{args.class_cond}_{epoch}.png",
                    ),
            ...
            if args.local_rank == 0:
                torch.save(
                    ...
                    os.path.join(
                        args.save_dir,
                        f"{args.arch}128_{args.dataset}-epoch_{250+args.epochs}-timesteps_{args.diffusion_steps}-class_condn_{args.class_cond}_{epoch}.pt",
                    ),
                )
                torch.save(
                    args.ema_dict,
                    os.path.join(
                        args.save_dir,
                        f"{args.arch}128_{args.dataset}-epoch_{250+args.epochs}-timesteps_{args.diffusion_steps}-class_condn_{args.class_cond}_ema_{args.ema_w}_{epoch}.pt",
                    ),
                )
